...
use CoreBundle\Repository\DirectoryRepository;
use CoreBundle\Services\CalculatorPriceService;
use CoreBundle\Services\Crawler\CrawlObserver;
use CoreBundle\Services\Crawler\CrawlOnlyDesiredUrls;
...
class BackLinksCheckerService
{
    ...
    /**
     * BackLinksCheckerService constructor.
     *
    ...
    public function __construct(
        ...
        $translator,
        $transactionService,
        $copywritingCalculatorPrice,
        $exchangeCalculatorPrice,
        $netlinkingService,
        $monolog
    ) {
    ...
    public function newCheck()
    {
        ...
        $directories = $directoryRepository->findDirectoriesWithNotFoundYetBackLinks();
        $netLinkingProjects = [];
        ...
        foreach ($directories as $directory) {
            ...
            foreach ($directory->getScheduleTasks() as $scheduleTask) {
                $job = $scheduleTask->getJob();
                $netLinkingProject = $job->getNetlinkingProject();
                $netLinkingProjects[] = $netLinkingProject;

                $backLinks[trim($netLinkingProject->getUrl(), '/')] = $job->getDirectoryBacklink();
            }
            ...
            $startingUrl = $directory->getNddTarget() ?? $directory->getUrl();
            $crawlProfile = new CrawlOnlyDesiredUrls($startingUrl, $backLinks);
            ...
            ;
            //NOTICE: to avoid crawling urls like this need to make own CrawlQueue and check ContentLength of response
            //http://www.hit-parade.com/faq.asp?p1=0&p2=0&p3=0&p4=&p5=0&p6=

            $this->monolog->info("Directory crawled", [
                'id' => $directory->getId(),
