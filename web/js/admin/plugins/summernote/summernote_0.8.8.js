  ...
  var Editor = function (context) {
    ...
    this.insertImage = function (src, param) {
      return async.createImage(src, param).then(function ($image) {
        ...
        // var figureNode = $('<figure>').append($image);
    ...
    this.floatMe = this.wrapCommand(function (value) {
      ...
      if ($target.parent('figure').length) {
          $target = $target.parent('figure');
      }
      $target.toggleClass('alignleft', value === 'left');
      $target.toggleClass('alignright', value === 'right');
      $target.toggleClass('aligncenter', value === 'none');
    });
    ...
    this.resize = this.wrapCommand(function (value) {
      var $target = $(this.restoreTarget());
      var imageSize = {
          width: value * $target.prop('naturalWidth'),
          height: value * $target.prop('naturalHeight'),
      };
      if ($target.parent('figure').length) {
          var figure = $target.parent('figure');
          figure.width(imageSize.width);
      }
      $target[0].width = imageSize.width;
      $target[0].height = imageSize.height  ;
    });
